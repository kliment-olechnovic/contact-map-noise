...
std::vector<std::string> generate_results(const std::string& input_data, const std::string& options)
{
	...
	if(!result_grouped_by_residue.grouped_contacts_summaries.empty())
	{
...

		{
			...
			if(compute_only_inter_chain_contacts)
			{
				plot_options_config.insert("compact");
			}
...

			{
				...
				{
					all_good=all_good && plotter.add_contact(i, result.contacts_summaries, spheres_input_result.sphere_labels, 0);
				}
				if(!all_good)
				{
					results.push_back(std::string("error"));
					results.push_back(std::string("Error : failed to plot contacts"));
					return results;
				}
				else if(!plotter.write_to_file(output, plot_options_config))
				{
					results.push_back(std::string("error"));
					results.push_back(std::string("Error : failed to write plot of contacts to string"));
					return results;
				}
			}

			results.push_back(std::string("residue-level-contact-map"));
